 ...
 var appTools = (function(){
	...
	var handlerId = {
		...
		thesaurusSuggest: function(response) {
			...
			NS.selectNode['equipments'].clear();

			for (var i in response) {
				NS.selectNode['equipments'].add(i, i);
			}

			var synKey = NS.selectNode['equipments'].getInputElement().getChildren().$[0].value;
			NS.selectNode['equipments'].getInputElement().getChildren().$[0].selected = true;
